...
import { useScenarioResults } from './scenario/useScenarioResults';
import AccordionItem from './scenario/AccordionItem';
...
const ScenarioDashboard: React.FC<ScenarioDashboardProps> = ({ groups, wells, scenarios, setScenarios }) => {
  ...
  const { scenarioResults, cfChartData } = useScenarioResults(groups, wells, scenarios);
              ...
              {isClassic ? (
                <>
                  <div className="sc-panelTitlebar sc-titlebar--red px-5 py-4 flex justify-between items-center">
                    ...
                    <button type="button" onClick={handleAddScenario} className="sc-btnPrimary text-[9px] px-4 py-2 rounded-lg font-black uppercase tracking-widest transition-all">
                      + NEW
                    </button>
                  </div>
                  <div className="p-4">
                    <div className="sc-insetDark rounded-lg p-3 space-y-3">
                      {scenarios.map(s => (
                        <button
                          ...
                          className={`group p-3 rounded-lg border cursor-pointer transition-all w-full text-left ${
                            s.id === activeScenarioId ? 'border-theme-warning bg-black/25' : 'border-black/25 bg-black/10 hover:bg-black/20'
                          }`}
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-x-3 min-w-0">
                              <div className="size-2.5 rounded-full" style={{ backgroundColor: s.color, boxShadow: `0 0 10px ${s.color}66` }}></div>
                            ...
                            <div className="size-1.5 rounded-full bg-black/20 group-hover:bg-black/40 animate-pulse"></div>
                          </div>
                ...
                <>
                  <div className="flex justify-between items-center mb-6">
                      ...
                      <button type="button" onClick={handleAddScenario} className="text-[9px] px-4 py-2 rounded-lg text-theme-bg font-black uppercase tracking-widest hover:shadow-glow-cyan transition-all bg-theme-cyan">
                          + NEW
                      </button>
                  </div>
                  <div className="space-y-3">
                      {scenarios.map(s => (
                          <button
                            ...
                            type="button"
                            onClick={() => { setActiveScenarioId(s.id); setEditingScenario(true); }}
                            className={`group p-4 rounded-inner border cursor-pointer transition-all w-full text-left ${s.id === activeScenarioId ? 'bg-theme-surface2 border-theme-magenta shadow-glow-magenta' : 'bg-theme-bg border-theme-border hover:border-theme-cyan hover:scale-[1.02]'}`}
                          >
                              <div className="flex items-center justify-between mb-3">
                                  ...
                                  <div className="size-1.5 rounded-full bg-theme-cyan/20 group-hover:bg-theme-cyan animate-pulse"></div>
                              </div>
	              ...
	              <div className="theme-transition">
                       ...
                       {isClassic ? (
                         <>
                           ...
                           <div className="p-4">
                             <div className="sc-insetDark rounded-lg p-4">
                               <div className="mb-4">
                                   <label htmlFor="sc-model-name-classic" className={labelClass}>MODEL NAME</label>
                                   <input id="sc-model-name-classic" type="text" value={activeScenario.name} onChange={e => updateScenario(activeScenario.id, { name: e.target.value.toUpperCase() })} className={inputClass} />
                               </div>
                               <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="sc-indicator-classic" className={labelClass}>INDICATOR</label>
                                        <input id="sc-indicator-classic" type="color" value={activeScenario.color} onChange={e => updateScenario(activeScenario.id, { color: e.target.value })} className="w-full h-10 rounded-lg cursor-pointer sc-inputNavy" />
                                    </div>
                               </div>
                             </div>
                           </div>
                         ...
                         <>
                           ...
                           <div className="mb-4">
                               <label htmlFor="sc-model-name" className={labelClass}>MODEL NAME</label>
                               <input id="sc-model-name" type="text" value={activeScenario.name} onChange={e => updateScenario(activeScenario.id, { name: e.target.value.toUpperCase() })} className={inputClass} />
                           </div>
                           <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label htmlFor="sc-indicator" className={labelClass}>INDICATOR</label>
                                    <input id="sc-indicator" type="color" value={activeScenario.color} onChange={e => updateScenario(activeScenario.id, { color: e.target.value })} className="w-full h-10 bg-theme-bg border border-theme-border rounded-lg cursor-pointer" />
                                </div>
                           </div>
                         </>
                       )}
                       ...
                       <AccordionItem title="Pricing" isOpen={openSection === 'PRICING'} onClick={() => setOpenSection('PRICING')} useBrandFont={theme.features.brandFont}>
                           <div className="grid grid-cols-2 gap-4">
                               <div><label htmlFor="sc-oil-price" className={labelClass}>OIL PRICE</label><input id="sc-oil-price" type="number" value={activeScenario.pricing.oilPrice} onChange={e => updatePricing(activeScenario.id, 'oilPrice', parseFloat(e.target.value))} className={inputClass} /></div>
                               <div><label htmlFor="sc-gas-price" className={labelClass}>GAS PRICE</label><input id="sc-gas-price" type="number" value={activeScenario.pricing.gasPrice} onChange={e => updatePricing(activeScenario.id, 'gasPrice', parseFloat(e.target.value))} className={inputClass} /></div>
                               <div><label htmlFor="sc-oil-diff" className={labelClass}>OIL DIFF</label><input id="sc-oil-diff" type="number" value={activeScenario.pricing.oilDifferential} onChange={e => updatePricing(activeScenario.id, 'oilDifferential', parseFloat(e.target.value))} className={inputClass} /></div>
                               <div><label htmlFor="sc-gas-diff" className={labelClass}>GAS DIFF</label><input id="sc-gas-diff" type="number" value={activeScenario.pricing.gasDifferential} onChange={e => updatePricing(activeScenario.id, 'gasDifferential', parseFloat(e.target.value))} className={inputClass} /></div>
                           </div>
                       </AccordionItem>
                       ...
                       <AccordionItem title="Fleet Scheduling" isOpen={openSection === 'SCHEDULE'} onClick={() => setOpenSection('SCHEDULE')} useBrandFont={theme.features.brandFont}>
                           <div className="space-y-6">
                               <div className="grid grid-cols-2 gap-4">
                                    <div><label htmlFor="sc-drill-days" className={labelClass}>DRILL DAYS</label><input id="sc-drill-days" type="number" value={activeScenario.schedule.drillDurationDays} onChange={e => updateScheduleParam(activeScenario.id, 'drillDurationDays', parseFloat(e.target.value))} className={inputClass} /></div>
                                    <div><label htmlFor="sc-stim-days" className={labelClass}>STIM DAYS</label><input id="sc-stim-days" type="number" value={activeScenario.schedule.stimDurationDays} onChange={e => updateScheduleParam(activeScenario.id, 'stimDurationDays', parseFloat(e.target.value))} className={inputClass} /></div>
                               </div>
                               <div>
                                   <span className={labelClass}>ANNUAL ALLOCATION (Y1-Y5)</span>
                                   <div className="grid grid-cols-5 gap-2">
                                       {activeScenario.schedule.annualRigs.slice(0,5).map((count, idx) => (
                                           <div key={`rig-y${idx + 1}`} className="text-center">
                                               <span className="text-[9px] text-theme-muted font-black block mb-1">Y{idx+1}</span>
                                               <input
                                                 type="number"
                                                 min="0"
                                                 aria-label={`Year ${idx + 1} rig count`}
                                                 value={count}
                       ...
                       <AccordionItem title="Risk Scalars" isOpen={openSection === 'SCALARS'} onClick={() => setOpenSection('SCALARS')} useBrandFont={theme.features.brandFont}>
                           <div className="space-y-6">
                               <div>
                                   <div className="flex justify-between mb-2"><label htmlFor="sc-capex-scalar" className={labelClass}>CAPEX MULTIPLIER</label><span className="text-[10px] font-black text-theme-cyan">{(activeScenario.capexScalar * 100).toFixed(0)}%</span></div>
                                   <input
                                     id="sc-capex-scalar"
                                     type="range"
                                     min="0.5"
                               ...
                               <div>
                                   <div className="flex justify-between mb-2"><label htmlFor="sc-prod-scalar" className={labelClass}>RECOVERY MULTIPLIER</label><span className="text-[10px] font-black text-theme-magenta">{(activeScenario.productionScalar * 100).toFixed(0)}%</span></div>
                                   <input
                                     id="sc-prod-scalar"
                                     type="range"
                                     min="0.5"
      ...
      <div className="xl:col-span-9 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              {scenarioResults.map((res) => (
                      ...
                      classicWrapperClassName="sc-panel theme-transition overflow-hidden group text-left min-h-[104px]"
                      modernWrapperClassName="rounded-panel border px-4 py-3 relative overflow-hidden theme-transition shadow-card group bg-theme-surface1/78 border-theme-border hover:border-theme-cyan text-left min-h-[104px]"
                      decorations={
                        <>
                          <div className="absolute top-0 left-0 w-1 h-full opacity-70" style={{ backgroundColor: res.scenario.color }}></div>
                          <div className="absolute top-0 right-0 size-20 blur-[42px] opacity-10 pointer-events-none" style={{ backgroundColor: res.scenario.color }}></div>
                        </>
                      }
                      title={res.scenario.name}
                      classicTitleAs="h4"
                      classicTitleClassName={`text-[9px] font-black uppercase tracking-[0.24em] text-white truncate ${theme.features.brandFont ? 'brand-font' : ''}`}
                      classicHeaderClassName="sc-panelTitlebar sc-titlebar--red px-4 py-2 flex items-center min-w-0"
                      classicBodyClassName="px-4 py-3"
                      modernHeader={
	          ...
	          <div className="grid grid-cols-1 2xl:grid-cols-2 gap-6">
	                <ScenarioCard
                    as="div"
                    classicWrapperClassName="sc-panel theme-transition overflow-hidden"
                    modernWrapperClassName="rounded-panel border p-8 shadow-card theme-transition bg-theme-surface1/60 border-theme-border"
                    title="PORTFOLIO OVERLAY"
                    classicTitleAs="h3"
                    classicTitleClassName={`text-[10px] font-black uppercase tracking-[0.4em] text-white ${theme.features.brandFont ? 'brand-font' : ''}`}
                    classicHeaderClassName="sc-panelTitlebar sc-titlebar--red px-5 py-4"
                    classicBodyClassName="p-5"
                    modernHeader={
                      <h3 className={`text-[11px] font-black uppercase tracking-[0.4em] mb-8 flex items-center gap-3 text-theme-lavender ${theme.features.brandFont ? 'brand-font' : ''}`}>
                        <span className="size-1.5 rounded-full bg-theme-magenta shadow-glow-magenta"></span>
                        PORTFOLIO OVERLAY
                      </h3>
                    }
                    modernBodyClassName=""
                  >
                      {groups.length === 0 || groups.every(g => g.wellIds.size === 0) ? (
                        <div className="text-center py-8 space-y-3">
                          <div className="size-16 mx-auto rounded-full border-2 border-theme-border/40 flex items-center justify-center">
                            <svg className="size-8 text-theme-muted/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <div className="space-y-6">
                    {isClassic ? (
                      <div className="sc-insetDark rounded-lg p-3 flex items-center gap-x-4 justify-end">
                      ...
                      <div className="flex items-center gap-x-4 justify-end pr-2">
